...
static ulonglong key_block_no(TABLE *table, uint keyno, ha_rows keyentry_pos)
{
  ...
  if (table->file->ha_is_clustering_key(keyno))
                                    ...
                                    Lifo_buffer *buf)
{
  ...
  */
  if (file->pushed_rowid_filter &&
      file->ha_is_clustering_key(file->get_table()->s->primary_key))
  ...
  */
  ...
  if (do_sort_keys)
  {
    ...
    if (strategy != index_strategy)
    {
      ...
      if (h_idx->ha_is_clustering_key(pk))
      {
                                ...
                                uint mrr_flags)
{
  return MY_TEST((mrr_flags & HA_MRR_SINGLE_POINT) &&
                 primary_file->ha_is_clustering_key(keyno) &&
                 optimizer_flag(thd, OPTIMIZER_SWITCH_MRR_SORT_KEYS));
}
