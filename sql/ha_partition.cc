...
bool ha_partition::is_clustering_key(uint index) const
{
  /* For now, assume that all tables are the same */
  return m_file[0]->is_clustering_key(index);
}
...
int ha_partition::index_init(uint inx, bool sorted)
{
  ...
  if (ha_is_clustering_key(table->s->primary_key))
  {
    /*
